...
auto Graph::add_node(std::unique_ptr<Node> node) -> void {
...
auto Graph::get_nodes() -> std::vector<std::unique_ptr<Node>>& { return nodes; }

auto Graph::get_node_by_uuid(const UUID& uuid) -> Node* {
  auto it = std::find_if(nodes.begin(), nodes.end(),
                         [&uuid](const std::unique_ptr<Node>& node) {
...
auto Graph::remove_link_by_uuid(const UUID uuid) -> void {
  auto it = std::remove(links.begin(), links.end(), uuid);
  links.erase(it);
}
...
auto Graph::add_item(std::unique_ptr<GraphItem> item) -> void {
...
auto Graph::remove_item_by_uuid(UUID uuid) -> void {
  auto it = std::remove_if(items.begin(), items.end(),
                           [&](const std::unique_ptr<GraphItem>& item) {
